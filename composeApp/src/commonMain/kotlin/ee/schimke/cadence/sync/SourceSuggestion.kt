...
interface SourceSuggestionsProvider {
    ...
    /**
     * Opens the suggested app if installed, otherwise launches the Play Store
     * (or the web Play Store as a fallback).
     */
...
/**
 * Curated catalogue of widely-used apps that register a SAF DocumentsProvider.
 * Once installed, each appears in the standard "Add directory" picker without
 * any extra integration on Shokz' side.
 */
object SourceSuggestionsCatalogue {
    val all: List<SourceSuggestion> = listOf(
        SourceSuggestion(
