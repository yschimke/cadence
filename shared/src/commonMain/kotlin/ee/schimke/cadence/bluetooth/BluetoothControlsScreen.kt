  ...
  ElevatedCard(modifier = Modifier.fillMaxWidth()) {
    Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
      ...
      Row(verticalAlignment = Alignment.CenterVertically) {
        Icon(
          imageVector =
            if (device != null) Icons.Filled.Bluetooth else Icons.Filled.BluetoothDisabled,
...
@Composable
private fun WorkingModeCard(selected: WorkingMode, onSelect: (WorkingMode) -> Unit) {
  ElevatedCard(modifier = Modifier.fillMaxWidth()) {
    Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
      Text(
        "Working mode",
    ...
    Column(modifier = Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
      ...
      Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        eqPresetCommands.forEach { cmd ->
          FilledTonalButton(onClick = { onAdvanced(cmd) }, modifier = Modifier.weight(1f)) {
        ...
        AdvancedCommand.entries
          ...
          .forEach { (category, commands) ->
            ...
            commands.forEach { cmd ->
              OutlinedButton(onClick = { onAdvanced(cmd) }, modifier = Modifier.fillMaxWidth()) {
                Text(cmd.label)
              }
            }
          }
